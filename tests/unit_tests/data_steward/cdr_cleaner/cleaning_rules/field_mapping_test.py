...
class FieldMappingTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.procedure_occurrence_fields = [
            ...
            'procedure_date', 'procedure_datetime', 'modifier_concept_id',
            'procedure_source_value'
        ]

        self.condition_date_fields = {
            '_start_datetime': 'condition_start_datetime',
        ...
        self.procedure_date_fields = {
            '_datetime': 'procedure_datetime',
            '_date': 'procedure_date'
        }
        self.drug_date_fields = {
            '_start_datetime': 'drug_exposure_start_datetime',
    ...
    def test_create_domain_field_dict(self, mock_get_domain,
        ...
        mock_get_domain_fields.side_effect = [
            self.condition_occurrence_fields, self.procedure_occurrence_fields
        ]
        ...
        self.assertCountEqual(condition_field_mappings[DOMAIN_DATE_FIELDS],
                              self.condition_date_fields)
        self.assertTrue(
            len(condition_field_mappings[DOMAIN_SPECIFIC_FIELDS]) == 0)
        ...
        self.assertCountEqual(procedure_field_mappings[DOMAIN_DATE_FIELDS],
                              self.procedure_date_fields)
        self.assertTrue(
            len(procedure_field_mappings[DOMAIN_SPECIFIC_FIELDS]) == 1)

    def test_resolve_common_field_mappings(self):
        actual = field_mapping.resolve_date_field_mappings(
            self.condition_common_fields, self.procedure_common_fields)
        expected = {
            'person_id': 'person_id',
    ...
    def test_resolve_date_field_mappings(self):
        actual = field_mapping.resolve_date_field_mappings(
            self.condition_date_fields, self.procedure_date_fields)
        expected = {
        ...
        actual = field_mapping.resolve_date_field_mappings(
            self.procedure_date_fields, self.condition_date_fields)
        expected = {
        ...
        actual = field_mapping.resolve_date_field_mappings(
            self.drug_date_fields, self.condition_date_fields)
        expected = {
    ...
    def test_resolve_specific_field_mappings(self, mock_get_field_mappings):
        mock_get_field_mappings.return_value = {
            'condition_test_field': 'procedure_test_field'
        }

        actual = field_mapping.resolve_specific_field_mappings(
        ...
        actual = field_mapping.resolve_specific_field_mappings(
            self.procedure_table, self.procedure_table, ['test_1', 'test_2'])
        expected = {'test_1': 'test_1', 'test_2': 'test_2'}
        self.assertDictEqual(actual, expected)

    @mock.patch(
        'cdr_cleaner.cleaning_rules.field_mapping.resolve_date_field_mappings')
    @mock.patch(
        'cdr_cleaner.cleaning_rules.field_mapping.resolve_specific_field_mappings'
    )
    @mock.patch(
        'cdr_cleaner.cleaning_rules.field_mapping.resolve_common_field_mappings'
    )
    def test_generate_field_mappings(self, mock_resolve_common_field_mappings,
                                     mock_resolve_specific_field_mappings,
                                     mock_resolve_date_field_mappings):
        mock_resolve_common_field_mappings.return_value = {
            'person_id': 'person_id',
            'procedure_concept_id': 'condition_concept_id',
            'procedure_source_value': 'condition_source_value',
        }
        mock_resolve_date_field_mappings.return_value = {
            'dest_date': 'src_date',
            'dest_datetime': 'src_datetime'
        }
        mock_resolve_specific_field_mappings.return_value = {
            'dest_field': 'src_field'
        }

        actual = field_mapping.generate_field_mappings(self.condition_table,
