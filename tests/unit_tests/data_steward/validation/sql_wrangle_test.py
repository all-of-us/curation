...
class SQLWrangleTest(unittest.TestCase):
    ...
    def test_get_temp_table_name(self):
        self.assertEqual(sql_wrangle.get_temp_table_name(self.query_1), 'temp.tempresults')
        self.assertEqual(sql_wrangle.get_temp_table_name(self.query_2), 'temp.rawdata_1006')

    def test_get_temp_table_query(self):
        self.assertTrue(sql_wrangle.get_temp_table_query(self.query_1).startswith('WITH rawdata'))
        self.assertTrue(
            sql_wrangle.get_temp_table_query(self.query_2).startswith('SELECT ce.condition_concept_id')
        )
