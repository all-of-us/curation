...
class CompletenessTest(unittest.TestCase):
    ...
    def test_get_cols(self, mock_query):
        ...
        mock_query.assert_called_with(
            consts.COLUMNS_QUERY_FMT.format(dataset_id=dataset_id))
        with patch('validation.participants.writers.bq_utils.response2rows'
                  ) as mock_response2rows:
    ...
    def test_create_completeness_query(self):
        ...
        expected_q1 = consts.COMPLETENESS_SUBQUERY_FMT.format(
            ...
            column_name=col1_name,
            concept_zero_expr='0')
        col2_concept_zero = consts.CONCEPT_ZERO_CLAUSE.format(
            column_name=col2_name)
        expected_q2 = consts.COMPLETENESS_SUBQUERY_FMT.format(
            dataset_id=dataset_id,
            table_name=table_name,
            omop_table_name=omop_table_name,
            table_row_count=table_row_count,
        ...
        expected_result = consts.COMPLETENESS_QUERY_FMT.format(
            union_all_subqueries=union_all_subqueries)
        actual_result = completeness.create_completeness_query(
            dataset_id, [col1, col2])
    ...
    def test_get_standard_table_name(self):
        ...
        t = completeness.get_standard_table_name(self.hpo_id + '_' +
                                                 device_cost)
        self.assertEqual(device_cost, t)
