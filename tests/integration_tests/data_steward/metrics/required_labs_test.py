...
class RequiredLabsTest(unittest.TestCase):
    ...
    def test_get_lab_concept_summary_query(self):
        ...
        unique_ancestor_concept_query = sql_wrangle.qualify_tables(
            """SELECT DISTINCT ancestor_concept_id FROM `{project_id}.{dataset_id}.{table_id}`""".format(
                project_id=self.project_id,
                dataset_id=self.dataset_id,
                table_id=required_labs.MEASUREMENT_CONCEPT_SETS_DESCENDANTS_TABLE))
        ...
        unique_measurement_concept_id_query = '''
                ...
                '''.format(project_id=self.project_id, dataset_id=self.dataset_id,
                           measurement_concept_sets_descendants=required_labs.MEASUREMENT_CONCEPT_SETS_DESCENDANTS_TABLE,
                           measurement=bq_utils.get_table_id(FAKE_HPO_ID, common.MEASUREMENT))
        ...
        self.assertEqual(int(expected_total_labs), int(actual_total_labs), msg='Compare the total number of labs')
        self.assertEqual(int(unique_measurement_concept_id_total_labs), len(submitted_labs), msg='Compare the number '
                                                                                                 'of labs submitted '
                                                                                                 'in the measurement')
