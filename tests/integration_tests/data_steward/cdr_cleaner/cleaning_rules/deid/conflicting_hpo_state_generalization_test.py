...
class ConflictingHpoStateGeneralizeTest(BaseTest.CleaningRulesTestBase):
    ...
    def test_conflicting_hpo_id(self):
        ...
        tables_and_counts = [{
            ...
            # the following tables are created when `setup_rule` runs, so they will break the sandboxing check
            # that happens during 'default_test()' run.
            # To get around the check, we can declare that these tables are created before the rule runs
            # and this is expected.
            # This also helps in cleanup that happens after test.
