...
| condition\_concept\_id         | Yes                     | integer     | A standard, valid, OMOP concept ID for the associated domain. The SNOMED terminology is considered standard for the condition domain. It is derived from the condition\_source\_concept\_id. If your source data does not use SNOMED codes (i.e. in `condition\_source\_concept\_id`) then they need to be translated. The query in [condition_concept_id.sql](https://github.com/cumc-dbmi/pmi_sprint_reporter/blob/master/resources/valid_concepts/condition_occurrence/condition_concept_id.sql) demonstrates how you may go about this. |
...
| condition\_end\_datetime       | Yes                     | datetime    | The date and time when the instance of the Condtion is considered to have ended. *See [datetime](index.html#datetime)* |
...
| condition\_source\_value       | Yes                     | varchar     | The source code for the condition as it appears in the source data. **NOTE**: This would be a ICD9, ICD10, or SNOMED code associated with a visit. |
...
| drug\_exposure\_start\_datetime | Yes                     | datetime    | The start date and time for the current instance of Drug utilization. Valid entries include a start date of a prescription, the date a prescription was filled, or the date on which a Drug administration procedure was recorded.                                                                                                                 
...
| drug\_exposure\_end\_datetime   | No                      | datetime    | The end date and time for the current instance of Drug utilization. It is not available from all sources.                                                                                                                                                                                                                                          
