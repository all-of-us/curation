...
SANDBOX_NLP = JINJA_ENV.from_string("""
...
FROM `{{project_id}}.{{dataset_id}}.condition_occurrence_ext` e
WHERE e.note_nlp_id IS NOT NULL)
""")
...
class FilterNLPfromDomains(BaseCleaningRule):
    ...
    def __init__(self,
        ...
        """
        desc = 'Removes NLP records note_nlp and condition_occurrence table'
    ...
    def get_query_specs(self, *args, **keyword_args) -> query_spec_list:
        ...
        sandbox_query = dict()
        sandbox_query[cdr_consts.QUERY] = SANDBOX_NLP.render(
        ...
        suppress_query = dict()
        suppress_query[cdr_consts.QUERY] = DELETE_NLP.render(
            project_id=self.project_id,
            dataset_id=self.dataset_id,
            sandbox_dataset_id=self.sandbox_dataset_id,
            sandbox_table=self.get_sandbox_tablenames()[0])
        ...
        clean_up_query = dict()
        clean_up_query[cdr_consts.QUERY] = DELETE_NLP_COND.render(
            project_id=self.project_id,
            dataset_id=self.dataset_id,
            sandbox_dataset_id=self.sandbox_dataset_id,
            sandbox_table=self.get_sandbox_tablenames()[0])
        queries_list.append(clean_up_query)
