...
from (select cast(stratum_1 as int64) stratum_1, count_value from `@results_database_schema.achilles_results` where analysis_id = 400 GROUP BY analysis_id, stratum_1, count_value) ar1
	inner join
	(select cast(stratum_1 as int64) stratum_1, count_value from `@results_database_schema.achilles_results` where analysis_id = 401 GROUP BY analysis_id, stratum_1, count_value) ar2
