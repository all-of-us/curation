...
from (select cast(stratum_1 as int64) stratum_1, count_value from `@results_database_schema.achilles_results` where analysis_id = 1000 GROUP BY analysis_id, stratum_1, count_value) ar1
	inner join
	(select cast(stratum_1 as int64) stratum_1, avg_value from `@results_database_schema.achilles_results_dist` where analysis_id = 1007 GROUP BY analysis_id, stratum_1, avg_value) ar2
