...
from (select analysis_id, cast(stratum_1 as int64) stratum_1 from `@results_database_schema.achilles_results` where analysis_id = 108 GROUP BY analysis_id, stratum_1) ar1
inner join
(
	select analysis_id, cast(stratum_1 as int64) stratum_1, count_value from `@results_database_schema.achilles_results` where analysis_id = 108 GROUP BY analysis_id, stratum_1, count_value
