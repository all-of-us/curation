...
# Create datasets
bq mk --dataset --description "Test RDR dataset for ${USERNAME}" --label "test:" ${APPLICATION_ID}:${RDR_DATASET_ID}
bq mk --dataset --description "Test COMBINED dataset for ${USERNAME}" --label "test:" ${APPLICATION_ID}:${COMBINED_DATASET_ID}
bq mk --dataset --description "Test EHR dataset for ${USERNAME}" --label "test:" ${APPLICATION_ID}:${BIGQUERY_DATASET_ID}
bq mk --dataset --description "Test EHR union dataset for ${USERNAME}" --label "test:" ${APPLICATION_ID}:${UNIONED_DATASET_ID}
