...
def _filter_stale_buckets(buckets: list, first_n: int = None):
    ...
    for bucket in buckets:
        ...
        if first_n and n >= first_n: break
        
        if (now - bucket.time_created).days > 90 and len(list(CLIENT.list_blobs(bucket.name))) == 0:
            stale_buckets.append(bucket.name)
            LOGGER.info(f"{n}: stale_bucket={bucket.name}, time_created={bucket.time_created}.")
            n += 1
...
def main():
    ...
    stale_buckets = _filter_stale_buckets(buckets=CLIENT.list_buckets(), first_n=200)
