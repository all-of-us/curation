...
USAGE="tools/table_copy.sh [--source_app_id <SOURCE_APPLICATION_ID>] [--target_app_id <TARGET_APPLICATION_ID>] --source_dataset <SOURCE_DATASET> [--source_prefix <SOURCE_PREFIX:''>] --target_dataset <TARGET_DATASET> [--target_prefix <TARGET_PREFIX:''>]"
...
SOURCE_DATASET=[[ "${SOURCE_APPLICATION_ID}" ]] && "${SOURCE_APPLICATION_ID}:${SOURCE_DATASET}" || "${SOURCE_DATASET}"
TARGET_DATASET=[[ "${TARGET_APPLICATION_ID}" ]] && "${TARGET_APPLICATION_ID}:${TARGET_DATASET}" || "${TARGET_DATASET}"
