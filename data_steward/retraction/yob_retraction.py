...
JOIN  `{{project_id}}.{{dataset_id}}.person` p USING (person_id)
...
""")
q = query.render(project_id=project_id, dataset_id=dataset_id)
execute(client, q)
...
dataset_definition = {
    'clean': {
        ...
        'desc':
            f"Hot fix applied to {dataset_id}."
            f"--"
            f"{dataset_id}'s description for reference -> Certain records removed from {dataset_id} based on the retraction rule",
        'labels': {
            "owner": "curation",
            "phase": "clean",
...
q = query.render(project_id=project_id,
                 dataset_id=dataset_definition['clean']['name'])
execute(client, q)
