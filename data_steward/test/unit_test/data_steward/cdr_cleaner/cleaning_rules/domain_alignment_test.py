...
class DomainAlignmentTest(unittest.TestCase):
    ...
    def test_parse_reroute_domain_query(self,
        ...
        mock_get_domain_id_field.side_effect = [self.condition_occurrence_id,
                                                self.condition_occurrence_id,
                                                self.procedure_occurrence_id,
                                                self.condition_occurrence_id]

        actual_query = domain_alignment.parse_reroute_domain_query(project_id=self.project_id,
    ...
    def test_parse_mapping_id_query_for_same_domains(self,
        ...
        mock_get_domain_concept_id.side_effect = [self.condition_concept_id, self.procedure_concept_id]

        actual_query = domain_alignment.parse_domain_mapping_query_for_same_domains(self.project_id,
                                                                                    self.dataset_id)
        ...
        expected_query = domain_alignment.DOMAIN_REROUTE_INCLUDED_INNER_QUERY.format(
            ...
            domain='\'{}\''.format(
                '\',\''.join([self.condition, domain_alignment.METADATA_DOMAIN])))
        ...
        expected_query += domain_alignment.DOMAIN_REROUTE_INCLUDED_INNER_QUERY.format(
            ...
            domain='\'{}\''.format(
                '\',\''.join([self.procedure, domain_alignment.METADATA_DOMAIN])))
    ...
    def test_parse_domain_mapping_query_cross_domain(self,
        ...
        actual_query = domain_alignment.parse_domain_mapping_query_cross_domain(project_id=self.project_id,
                                                                                dataset_id=self.dataset_id,
                                                                                dest_table=self.condition_table)
    ...
    @mock.patch('cdr_cleaner.cleaning_rules.domain_mapping.value_requires_translation')
    @mock.patch('cdr_cleaner.cleaning_rules.domain_mapping.get_field_mappings')
    def test_resolve_field_mappings(self, mock_get_field_mappings, mock_value_requires_translation,
                                    mock_get_value_mappings):
        get_field_mappings_return_value = OrderedDict()
        get_field_mappings_return_value[self.procedure_concept_id] = self.condition_concept_id
        get_field_mappings_return_value[self.procedure_type_concept_id] = self.condition_type_concept_id
        ...
        select_field_1 = SRC_FIELD_AS_DEST_FIELD.format(src_field=self.condition_concept_id,
                                                        dest_field=self.procedure_concept_id)

        select_field_2 = CASE_STATEMENT.format(src_field=self.condition_type_concept_id,
        ...
        mock_get_field_mappings.assert_called_once_with(self.condition_table, self.procedure_table)

        mock_value_requires_translation.assert_any_call(self.condition_table, self.procedure_table,
                                                        self.condition_concept_id, self.procedure_concept_id)
        mock_value_requires_translation.assert_any_call(self.condition_table, self.procedure_table,
                                                        self.condition_type_concept_id,
                                                        self.procedure_type_concept_id)

        mock_get_value_mappings.assert_called_once_with(self.condition_table, self.procedure_table,
                                                        self.condition_type_concept_id,
                                                        self.procedure_type_concept_id)
    ...
    @mock.patch('cdr_cleaner.cleaning_rules.domain_alignment.parse_domain_mapping_query_for_excluded_records')
    @mock.patch('cdr_cleaner.cleaning_rules.domain_alignment.parse_domain_mapping_query_for_same_domains')
    @mock.patch('cdr_cleaner.cleaning_rules.domain_alignment.parse_domain_mapping_query_cross_domain')
    @mock.patch('cdr_cleaner.cleaning_rules.domain_alignment.bq_utils.create_standard_table')
    def test_get_domain_mapping_queries(self,
        ...
        mock_parse_domain_mapping_query_cross_domain.side_effect = [cross_domain_query_condition,
        ...
        mock_parse_domain_mapping_query_cross_domain.assert_any_call(self.project_id, self.dataset_id,
                                                                     self.condition_table)
        mock_parse_domain_mapping_query_cross_domain.assert_any_call(self.project_id, self.dataset_id,
                                                                     self.procedure_table)
        mock_parse_domain_mapping_query_for_same_domains.assert_called_once_with(self.project_id, self.dataset_id)
    ...
    def test_get_reroute_domain_queries(self,
        ...
        mock_parse_reroute_domain_query.side_effect = [reroute_domain_query_condition, reroute_domain_query_procedure]
