...
EHR_CONSENT_QUERY = (' WITH ordered_response AS\n'
                     ' (SELECT'
                     ' person_id,'
...
COPY_EHR_QUERY = ('      SELECT * FROM {ehr_dataset_id}.{table} t'
                  '      WHERE EXISTS'
                  '           (SELECT 1 FROM {ehr_rdr_dataset_id}.{ehr_consent_table_id} c'
                  '            WHERE t.person_id = c.person_id)')

MAPPING_QUERY_WITH_PERSON_CHECK = ('SELECT DISTINCT'
...
MAPPING_QUERY_WITHOUT_PERSON_CHECK = ('SELECT DISTINCT'
                                      ' \'{rdr_dataset_id}\'  AS src_dataset_id,'
                                      '  {domain_table}_id  AS src_{domain_table}_id,'
                                      '  \'rdr\' as src_hpo_id,'
                                      '  {domain_table}_id + {mapping_constant}  AS {domain_table}_id'
