...
EHR_PERSON_VALUES = ('SELECT person_id, {field} '
                     'FROM `{project}.{dataset}.{table}` ')
...
PII_CODES_LIST = [
    str(id_match.OBS_PII_NAME_FIRST),
    str(id_match.OBS_PII_NAME_MIDDLE),
    str(id_match.OBS_PII_NAME_LAST),
    str(id_match.OBS_PII_EMAIL_ADDRESS),
    str(id_match.OBS_PII_PHONE),
    str(id_match.OBS_PII_STREET_ADDRESS_ONE),
    str(id_match.OBS_PII_STREET_ADDRESS_TWO),
    str(id_match.OBS_PII_STREET_ADDRESS_CITY),
    str(id_match.OBS_PII_STREET_ADDRESS_STATE),
    str(id_match.OBS_PII_STREET_ADDRESS_ZIP),
    str(id_match.OBS_PII_BIRTH_DATETIME),
    str(id_match.OBS_PII_SEX)
]

# Select PII table values.
PII_VALUES = ('SELECT person_id, {field} '
              'FROM `{project}.{dataset}.{hpo_site_str}{table_suffix}`')

PII_LOCATION_VALUES = ('SELECT location_id, {field} '
