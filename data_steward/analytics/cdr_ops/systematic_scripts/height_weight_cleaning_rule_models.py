...
-- excluding weird weights (e.g. 'fetal')
{caveat_string}
...
bin_borders = [80, 100, 120, 140, 160, 180, 200, 220, 240, 260, 280, 300, 320, 340, 360, 400]
...
query_all_weights = get_all_descendant_concepts_as_table + query_all_weights
...
# # Checking - why are we losing so many people with respect to height? What are the IDs that are the basis for exclusion?

height_exclusion_conditions = f"""
SELECT
DISTINCT
c.concept_id, c.concept_name, COUNT(co.person_id) as num_persons
FROM
`{DATASET}.unioned_ehr_condition_occurrence` co
LEFT JOIN
`{DATASET}.concept` c
...
WHERE
c.concept_id IN (80502, 77079, 4078547, 4165513,
      4027406, 2766929, 4263905, 4192565, 4210444, 4219032, 4058850, 4023190, 4205235, 2767168,
      4239226, 40482475, 4162099, 4239226, 2767165, 2859839, 2777592, 4105090, 45766159, 321661,
      2799062, 2777583, 2784242, 4070317, 4203318, 37017415, 2767188, 4069953, 4242396, 2777581,
      2105109, 2102534, 4078713, 2777590, 2767167, 4261484, 4196391, 2767152, 2000074, 4260962,
      ...
      2847077, 2806601, 2819463, 2819475, 2767192, 2767146, 2767172, 2766941, 2767408, 2767411,
      2767170, 2767404, 2767198, 2767151, 2767137, 2767180, 2767203, 2767187)
GROUP BY c.concept_id, c.concept_name, co.person_id
ORDER BY num_persons DESC
"""

exclusion_heights = pd.io.gbq.read_gbq(height_exclusion_conditions, dialect='standard')

x = exclusion_heights['num_persons'].tolist()

exclusion_heights

height_persons_exc = persons_to_exclude_height_df['outlier_patient'].tolist()

# ## Checking Osteoporosis
...
WHERE
LOWER(c.concept_name) LIKE '%osteoporosis%'
GROUP BY co.person_id
"""

those_with_osteo = pd.io.gbq.read_gbq(another_test, dialect = "standard")

num_with_osteo = sum(those_with_osteo['count'])

query_osteo_heights = f"""
SELECT
m.value_as_number, {height_conversion_rules_str}
FROM
`{DATASET}.unioned_ehr_condition_occurrence` co
...
osteo_height_df = pd.io.gbq.read_gbq(query_osteo_heights, dialect='standard')

# +
osteo_heights = osteo_height_df['value_as_number'].tolist()

osteo_number_records = str(len(osteo_heights))
osteo_mean = str(round(np.mean(osteo_heights), rounding_val))

osteo_decile1 = str(round(np.percentile(osteo_heights, 10), rounding_val))

osteo_quartile1 = str(round(np.percentile(osteo_heights, 25), rounding_val))
osteo_median = str(round(np.percentile(osteo_heights, 50), rounding_val))
osteo_quartile3 = str(round(np.percentile(osteo_heights, 75), rounding_val))

osteo_decile9 = str(round(np.percentile(osteo_heights, 90), rounding_val))

osteo_stdev = str(round(np.std(np.asarray(osteo_heights)), rounding_val))

osteo_min_weight = min(osteo_heights)
osteo_max_weight = max(osteo_heights)

# +
oseto_height_attributes = f"""
Number of heights: {osteo_number_records}

Minimum: {osteo_min_weight}
maximum: {osteo_max_weight}

Mean Height: {osteo_mean}
Standard Devidation: {osteo_stdev}

10th Percentile: {osteo_decile1}
25th Percentile: {osteo_quartile1}
Median: {osteo_median}
75th Percentile: {osteo_quartile3}
90th Percentile: {osteo_decile9}
"""

print(oseto_height_attributes)
...
# ### Checking Spinal Stenosis in Cervical Region
...
co.condition_concept_id = c.concept_id
WHERE
LOWER(c.concept_name) LIKE '%spinal stenosis in cervical region%'
GROUP BY co.person_id
"""

those_with_spinal_stenosis = pd.io.gbq.read_gbq(another_test2, dialect = "standard")

num_with_spinal_sten = sum(those_with_spinal_stenosis['count'])
...
m.value_as_number <> 0.0  -- not something we expect; appears for a site
AND
LOWER(c.concept_name) LIKE '%spinal stenosis of lumbar region%'
ORDER BY value_as_number DESC
"""

ss_height_df = pd.io.gbq.read_gbq(query_ss_heights, dialect='standard')

# +
ss_heights = ss_height_df['value_as_number'].tolist()

ss_number_records = str(len(ss_heights))
ss_mean = str(round(np.mean(ss_heights), rounding_val))

ss_decile1 = str(round(np.percentile(ss_heights, 10), rounding_val))

ss_quartile1 = str(round(np.percentile(ss_heights, 25), rounding_val))
ss_median = str(round(np.percentile(ss_heights, 50), rounding_val))
ss_quartile3 = str(round(np.percentile(ss_heights, 75), rounding_val))

ss_decile9 = str(round(np.percentile(ss_heights, 90), rounding_val))

ss_stdev = str(round(np.std(np.asarray(ss_heights)), rounding_val))

ss_min_weight = min(ss_heights)
ss_max_weight = max(ss_heights)

# +
ss_height_attributes = f"""
Number of heights: {ss_number_records}

Minimum: {ss_min_weight}
maximum: {ss_max_weight}

Mean Height: {ss_mean}
Standard Devidation: {ss_stdev}

10th Percentile: {ss_decile1}
25th Percentile: {ss_quartile1}
Median: {ss_median}
75th Percentile: {ss_quartile3}
90th Percentile: {ss_decile9}
"""

print(ss_height_attributes)
...
# ## Suggestion: Given the sheer prevalance of some conditions (namely osteoporosis and spinal stenosis of lumbar region), we should not filter out the participants who suffer from common conditions.
#
# ### This exclusion, however, should only apply if the set of participants with the particular condition do not differ significantly from the overall set of participants.

# # Let's also see why people are getting excluded from weight

weight_exclusion_conditions = f"""
SELECT
DISTINCT
c.concept_id, c.concept_name, COUNT(co.person_id) as num_persons
FROM
...
ON co.condition_concept_id = c.concept_id
WHERE
c.concept_id IN (440076, 436675, 4204347, 4229881, 36714252, 37116399, 36676691,
      37395978, 35622248, 36714182, 4033951, 44784528, 36716141, 4214302, 37111630, 44783252, 36674971,
      36675008, 36716779, 37016351, 36713802, 36717597, 36713801, 4330231, 4052869, 4111319, 4082503,
      4238810, 36714112, 4006979, 4287262, 36713764, 37396322, 4096098, 4269485, 36674974, 37117816,
      36716442, 37110064, 4025818, 4216214, 4331598, 36715406, 37396271, 36674412, 4300305, 36713653,
...
exclusion_weights = pd.io.gbq.read_gbq(weight_exclusion_conditions, dialect='standard')

exclusion_weights

understand_anorexia_prevalence = f"""
SELECT
COUNT(DISTINCT co.person_id) as num_persons
FROM
`{DATASET}.unioned_ehr_condition_occurrence` co
LEFT JOIN
`{DATASET}.concept` c
...
co.condition_concept_id = c.concept_id
WHERE
LOWER(c.concept_name) LIKE '%anorexia nervosa%'
GROUP BY c.concept_name, co.person_id
ORDER BY num_persons DESC
"""

anorexia_participants = pd.io.gbq.read_gbq(understand_anorexia_prevalence, dialect='standard')

# +
# number of anorexic participants

anorexia_participants_num = sum(anorexia_participants['num_persons'].tolist())

print(f"""
There are {anorexia_participants_num} number of participants in the dataset who have at least one condition code for anorexia nervosa.
""")
# -

# ## Thoughts: we could exclude weights for participants while their condition (e.g. anorexia nervosa) is still active. This would prevent us from losing too many data points and would allow researchers to leverage these data points should they be interested in investigating eating disorders
