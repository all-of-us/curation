...
# #  Query 17 Check that wear_consent records are suppressed in the 'observation' and 'survey_conduct' tables in the deid dataset.
...
if df1['bad_rows'].sum()==0:
 df = df.append({'query' : 'Query17 wear_consent records are cleaned as expected.', 'result' : 'PASS'},  
                ignore_index = True) 
else:
 df = df.append({'query' : 'Query17 wear_consent records have not been cleaned as expected.', 'result' : 'Failure'},  
                ignore_index = True) 
df1
