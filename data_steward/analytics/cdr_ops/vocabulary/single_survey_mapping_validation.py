...
# # Purpose
# Validate proposed additions to the vocabulary. Odysseus will send CONCEPT and CONCEPT_RELATIONSHIP files
# that need to be reviewed. <br>
...
#
# Concept creation and mapping practices are known to differ between modules. Varify anything out of the ordinary
# and create new checks when required.
#

# # Requirements
# Before running the notebook ensure all data has been updated recently.
#
# 1. In the test project the dataset redcap_surveys should have been updated recently. Tables 'data_dictionaries',
# 'field_annotations' and 'branching_logic' are required.
# 2. Odysseus provided files are placed in the `vocabulary/data_storage/odysseus_proposals` folder.
# Recommendation: Include a date in the file names. There will most likely be versions of the proposals.
# EX: `20230831_new_concepts.csv`, `20230831_new_relationships.csv`
...
concept_file_name = ''
concept_relationship_file_name = ''
...
# +
# import odysseus mapping csvs
ody_concept = pd.read_csv(file_path + concept_file_name,
                          index_col=None,
                          header=0,
                          encoding='latin-1')

ody_cr = pd.read_csv(file_path + concept_relationship_file_name,
                     index_col=None,
                     header=0,
                     encoding='latin-1')
# -
...
current_vocabulary['concept_code'] = current_vocabulary[
    'concept_code'].str.lower()
...
FROM (
  ...
  AND field_type NOT IN ('text', 'descriptive')
  )
...
sql_statements = []
for form_dict in form_names:
    for full_name in form_dict:
        sql_statements.append(
            build_concept_relationship_query.format(
...
reverse_half_cr = reverse_half_cr.rename(columns={
    'concept_code_1': 'concept_code_2',
    'concept_code_2': 'concept_code_1'
})

# reverse relationships
reverse_half_cr['relationship_id'] = np.where(
    reverse_half_cr['relationship_id'].str.contains('Maps to'), 'Mapped from',
    reverse_half_cr['relationship_id'])
reverse_half_cr['relationship_id'] = np.where(
    reverse_half_cr['relationship_id'].str.contains('PPI parent code of'),
    'Has PPI parent code', reverse_half_cr['relationship_id'])
reverse_half_cr['relationship_id'] = np.where(
    reverse_half_cr['relationship_id'].str.contains('Has answer \(PPI\)'),
    'Answer of (PPI)', reverse_half_cr['relationship_id'])
...
mock_concept_relationship = concat_cr.drop_duplicates(
    keep='first', ignore_index=True).reset_index(drop=True)
print(len(mock_concept_relationship))

# final cleaning
mock_concept_relationship = mock_concept_relationship[
    ~mock_concept_relationship['concept_code_1'].isin(ignore_list) &
...
mock_fin = full_relationships[[
    'concept_code_1', 'concept_code_2', 'concept_id_x', 'concept_id_y',
    'relationship_id'
]]
mock_fin = mock_fin.rename(columns={
    'concept_id_x': 'concept_id_1',
    'concept_id_y': 'concept_id_2'
})
mock_fin
...
# Create a table that shows the status of each concept_code (accepted, concept_code_does not match dictionary,
# no matching concept_id)
# -
...
missing_ids = concept_qc_base[concept_qc_base['_merge'] ==
                              'left_only'].reset_index(drop=True)
...
agreement_ids = concept_qc_base[concept_qc_base['_merge'] ==
...
concept_w_status = concept_w_status.drop(
    columns=['status_x', 'status_y', 'status'])
concept_w_status = concept_w_status.rename(
    columns={
        'concept_code_x': 'concept_code',
        'status_comb': 'status',
        'concept_code_y': 'concept_code_lacking_id'
    })
...
concept_w_status = concept_w_status[
    ~concept_w_status['concept_code_lacking_id'].isin(ignore_list)]
concept_w_status = concept_w_status.drop_duplicates(ignore_index=True,
                                                    keep='first')
concept_w_status
# -
...
athena_join2['concept_code_exists_in_athena'] = np.where(
    (~athena_join2['concept_code_y'].isnull()) |
    (~athena_join2['concept_code'].isnull()), 'True', 'False')

# If concept_code_exists_in_athena = True and status = Accepted these need to go back to ody. They already have an ID
# in athena and should not be remapped.
# If concept_code_exists_in_athena = True and status = no_matching_concept_id these are known to already be mapped in
# Athena. Have broader conversation to make sure these are as they should be.
# this is just a view. Summary of what you can find in feedback_with_athena
already_exist_in_athena = athena_join2[
    athena_join2['concept_code_exists_in_athena'] == 'True']
already_exist_in_athena

# Concept check: Not mapped concepts, already mapped concepts, ids not unique

# +
feedback_with_athena = athena_join2[[
    'concept_id', 'concept_name_x', 'domain_id', 'vocabulary_id',
    'concept_class_id', 'standard_concept', 'concept_code_x',
    'valid_start_date', 'valid_end_date', 'invalid_reason',
    'concept_code_lacking_id', 'status', 'concept_code_exists_in_athena'
]]
feedback_with_athena = feedback_with_athena.rename(columns={
    'concept_name_x': 'concept_name',
    'concept_code_x': 'concept_code'
})
feedback_with_athena = feedback_with_athena.drop_duplicates(ignore_index=True,
                                                            keep='first')
...
# concept_qc_base is the aou generated concept file(left) with odysseus concept file(right)
class_ids_check = concept_qc_base[(concept_qc_base['concept_class_id_x'] !=
                                   concept_qc_base['concept_class_id_y']) &
...
vocabulary_ids_check = concept_qc_base[
    (concept_qc_base['vocabulary_id_x'] != concept_qc_base['vocabulary_id_y']) &
    (concept_qc_base['_merge'] == 'both')]
print(len(vocabulary_ids_check))

# ## concept check: standard status
#
# Odysseus will sometimes map concepts to standards in one of the last stages of concept creation. Most likely after
# AoU has approved the concepts(concept_codes/concept_names/concept_ids) in the concept file. This check is more of a
# reminder that the mappings to standard concepts should be checked. If the concept maps to
# itself ('Mapped from','Maps to') then it should be standard in the concept table. If it does not have those
...
standards_check = concept_qc_base[(concept_qc_base['standard_concept_x'] !=
                                   concept_qc_base['standard_concept_y']) &
                                  (concept_qc_base['_merge'] == 'both')]
...
missing_rel = missing_rel.sort_values(by='concept_id_1',
...
added_rel = added_rel.sort_values(by='concept_id_1',
                                  ascending=True).reset_index(drop=True)
...
approved_rel = approved_rel.sort_values(by='concept_id_1',
                                        ascending=True).reset_index(drop=True)
approved_rel

# +
# sanity check pop up analysis. Many of the results above were linked to these two concept_ids which are a known
# issue for odysseus to fix.
# look at the ones that are not linked to the ids
id_list = [903079.0, 903087.0]
issues_added = added_rel[(~added_rel['concept_id_1'].isin(id_list)) & (
    ~added_rel['concept_id_2'].isin(id_list))].reset_index(drop=True)
...
frank_cr = frank_merge_id_1.merge(slice_concept,
                                  ...
                                  right_on='concept_id',
                                  indicator=False)
frank_cr = frank_cr.rename(columns={
    'concept_code_x': 'concept_code_1',
    'concept_code_y': 'concept_code_2'
})
...
athena_join_cr2['athena_status'] = np.where(
    (~athena_join_cr2['concept_code_x'].isnull()),
    'concept_code_1 exists in athena', '')
athena_join_cr2['athena_status'] = np.where(
    (~athena_join_cr2['concept_code_y'].isnull()),
    'concept_code_2 exists in athena', athena_join_cr2['athena_status'])
athena_join_cr2['athena_status'] = np.where(
    (~athena_join_cr2['concept_code_x'].isnull()) &
    (~athena_join_cr2['concept_code_y'].isnull()),
    'both concept codes exist in athena', athena_join_cr2['athena_status'])

# insert athena codes into concept_code fields
athena_join_cr2['concept_code_1'] = np.where(
    (~athena_join_cr2['concept_code_x'].isnull()),
    athena_join_cr2['concept_code_x'], athena_join_cr2['concept_code_1'])
athena_join_cr2['concept_code_2'] = np.where(
    (~athena_join_cr2['concept_code_y'].isnull()),
    athena_join_cr2['concept_code_y'], athena_join_cr2['concept_code_2'])

# drop unnecessary fields
athena_join_cr2 = athena_join_cr2.drop(columns={
    'concept_code_x', 'concept_code_y', 'concept_id_x', 'concept_id_y'
})
athena_join_cr2
# -
...
approved_rel_cut = approved_rel_cut.rename(
    columns={'relationship_id_x': 'relationship_id'})
approved_rel_cut
...
# step 5 merge tables
cr_merge_approved = athena_join_cr2.merge(
    approved_rel_cut,
    how='outer',
...
cr_merge_approved['concept_code_1'] = cr_merge_approved[
    'concept_code_1_x'].combine_first(cr_merge_approved['concept_code_1_y'])
cr_merge_approved['concept_code_2'] = cr_merge_approved[
    'concept_code_2_x'].combine_first(cr_merge_approved['concept_code_2_y'])
cr_merge_approved = cr_merge_approved.drop(columns={
    'concept_code_1_x', 'concept_code_1_y', 'concept_code_2_x',
    'concept_code_2_y'
})

cr_merge_added = cr_merge_approved.merge(
    added_rel_cut,
    how='outer',
    left_on=['concept_id_1', 'concept_id_2', 'relationship_id'],
    right_on=['concept_id_1', 'concept_id_2', 'relationship_id'],
...
cr_merge_added['concept_code_1'] = cr_merge_added[
    'concept_code_1_x'].combine_first(cr_merge_added['concept_code_1_y'])
cr_merge_added['concept_code_2'] = cr_merge_added[
    'concept_code_2_x'].combine_first(cr_merge_added['concept_code_2_y'])
cr_merge_added['status'] = cr_merge_added['status_x'].combine_first(
    cr_merge_added['status_y'])
cr_merge_added = cr_merge_added.drop(
    columns={
        'concept_code_1_x', 'concept_code_1_y', 'concept_code_2_x',
        'concept_code_2_y', 'status_x', 'status_y'
    })

cr_merge_missing = cr_merge_added.merge(
    missing_rel_cut,
    how='outer',
    left_on=['concept_id_1', 'concept_id_2', 'relationship_id'],
    right_on=['concept_id_1', 'concept_id_2', 'relationship_id'],
...
cr_merge_missing['concept_code_1'] = cr_merge_missing[
    'concept_code_1_x'].combine_first(cr_merge_missing['concept_code_1_y'])
cr_merge_missing['concept_code_2'] = cr_merge_missing[
    'concept_code_2_x'].combine_first(cr_merge_missing['concept_code_2_y'])
cr_merge_missing['status'] = cr_merge_missing['status_x'].combine_first(
    cr_merge_missing['status_y'])
cr_merge_missing = cr_merge_missing.drop(
    columns={
        'concept_code_1_x', 'concept_code_1_y', 'concept_code_2_x',
        'concept_code_2_y', 'status_x', 'status_y'
    })
...
test = base_cr_with_status[
    (base_cr_with_status['status'] != 'Accepted') &
    (base_cr_with_status['status'] != 'aou generated might need to be added') &
    (base_cr_with_status['status'] != 'ody generated. manually review')]
test

# +
feedback_with_athena_cr = base_cr_with_status[[
    'concept_id_1', 'concept_id_2', 'concept_code_1', 'concept_code_2',
    'relationship_id', 'athena_status', 'status'
]]
feedback_with_athena_cr = feedback_with_athena_cr.drop_duplicates(
