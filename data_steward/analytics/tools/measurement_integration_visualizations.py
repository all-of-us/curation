...
ax.set_title("Complete Blood Count Differential (CBC Diff) Measurement Integration", size=14)
plt.savefig("cbc_diff_measurement_integration.jpg")
...
ax.set_title("Comprehensive Metabolic Panel (CMP) Measurement Integration", size=14)
plt.savefig("cmp_measurement_integration.jpg")
...
ax.set_title("Lipid Measurement Integration", size=14)
plt.savefig("lipid_measurement_integration.jpg")
...
# +
unit_file_hpo_sheet_name = 'sites_measurement_hpo_sheets_data_analytics.xlsx'
...
for sheet in hpo_sheets:
    sheet_cols = sheet.columns
    sheet_cols = sheet_cols[1:]  # first col does not have data
    new_df = pd.DataFrame(columns=sheet_cols)
...
plt.tight_layout()
img_name = name_of_interest + "_measurement_integration.jpg"
